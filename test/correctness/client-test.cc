...
int main() {
    ...
    int port = 8080;
    client::Client curr_client = client::Client(port, server_ip);
    std::ifstream file("sample_test.json");

    if (!file) {
        std::cerr << "Could not open sample_test.json" << std::endl;
        return 1;
    }
    ...
    for (const auto& test : j["tests"]) {
        ...
        auto output = curr_client.receive_output();
