  ...
  while (true) {
    ...
    for (int i = 0; i < n; ++i) {
      if (events[i].data.fd == server_listen_fd_) {
        ...
        ssize_t count = recvfrom(server_listen_fd_, &query, sizeof(query), 0,
        ...
        spdlog::info("Received query from {}:{}.", client_ip, client_port);

        helper::check_error(
            handle_trade_data_query(server_listen_fd_, query, client_addr) < 0,
...
int EpollServer::handle_trade_data_query(int sock, TradeDataQuery query, sockaddr_in client_addr) {
  ...
  ssize_t bytes_sent = sendto(sock, &result_size, sizeof(result_size), 0,
  ...
  // Send each item in the result
  if (t_not_r) {
    for (auto& data : tresult) {
      send_without_serialisation(sock, data, client_addr);
    }
  } else {
