...
int32_t main(int32_t argc, char *argv[]) {
...
#endif
  spdlog::info("Command-line Chat Client starting to connect to {}:{}",
               server_ip, port);
  ...
  try {
    ...
    std::cerr << "An unknown error occurred during client initialization."
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  while (g_client_running) {
    ...
    std::cin >> query.symbol_id >> query.start_time_point >>
    ...
    if (query.resolution == 0) {
      ...
      for (const TradeData &data : output) {
        std::cout << data.symbol_id << " " << data.created_at << " "
                  << data.trade_id << " " << data.price.price << "e"
                  << data.price.price_exponent << " " << data.quantity.quantity
                  << "e" << data.quantity.quantity_exponent << " "
      ...
      if (query.metrics & (1 << 0)) {
        ...
        for (const Result &data : output) {
          ...
          std::cout << "Timestamp: " << data.start_time
                    << "; Min Price: " << data.lowest_price.price << "e"
                    << low_exp << "; Max Price: " << data.highest_price.price
                    << "e" << high_exp << std::endl;
        }
      }
    }
